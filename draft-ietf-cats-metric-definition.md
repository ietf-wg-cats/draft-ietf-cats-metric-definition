...
This section gives an initial Registry Entry for a single normalized CTAS metric (CATS L2 metric). 
## Summary
...
### Name
NormFI_Active_CATS-L2_RFCXXXXsecY_Unitless_Singleton

Naming Rule Explanation
* NormFI: Metric type (Full Normalized Score)
...
* Unitless: No unit
* Singleton: Output type is a single normalized value
### URI
To-be-assigned.
###  Description
This metric is a single normalized score for CATS. It is generated by aggregating L0 and/or L1 level metrics of computing resources and network state followed by normalization. It is used to quickly evaluate the comprehensive capability of service instances and support the efficient execution of network-level traffic steering decisions. 
### Change Controller
IETF
### Version
1.0
...
### Reference Methods
Raw Metrics collection: Collect L0 service and compute raw metrics based on service instance management platform-specific protocols or tools (e.g., Prometheus in Kubernetes). Collect L0 network performance raw metrics based on exisitng protocols (e.g., NETCONF {{?RFC6241}}, IPFIX {{?RFC7011}}).   
...
The reference method aggregates and normalizes L0 metrics to generate L1 metrics in different categories, and further calculate a L2 singleton score for full normalization.
...
Sampling method: Continuous sampling (for example, collect L0 metrics every 10s for calculation.)
### Runtime Parameters and Data Format
CATS Service Contact Instance ID (CSCI-ID): an identifier of CATS SCI. According to {{I-D.ietf-cats-framework}}, a unicast IP address can be an example of the identifier. (format: ipv4-address-no-zone or ipv6-address-no-zone, complying with {{?RFC6991}})
...
Measurement_Window: Metric calculation time window (default 10s, format: decimal 64, floating point, unit: s)
### Roles
